...
import { Calendar, Filter, Download, RefreshCw, Droplets, Thermometer, Activity, Waves } from 'lucide-react';
...
export default function HistoryPage() {
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/sensors/latest?${params.toString()}`);
  ...
  const exportToCSV = () => {
    ...
    const rows = historyData.map(item => {
      ...
      if (filters.showPh) row.push(item.ph ?? '-');
      if (filters.showSuhu) row.push(item.suhu ?? '-');
      if (filters.showKekeruhan) row.push(item.kekeruhan ?? '-');
      if (filters.showKualitas) row.push(item.kualitas ?? '-');
      return row;
    });
